...
public class CascadeManagementService {
    ...
    public void cascadeDeleteSubClass(Integer subClassId, Integer targetSubClassId, boolean forceDelete) {
        ...
        List<Student> students = studentRepository.findByStuClassId(subClass.get());
        ...
        if (forceDelete) {
            ...
            for (Student student : students) {
                student.setStuClassId(targetSubClass.get());
    ...
    public void migrateStudents(List<Integer> studentIds, Integer targetSubClassId) {
        ...
        for (Integer studentId : studentIds) {
            ...
            if (student.isPresent()) {
                student.get().setStuClassId(targetSubClass.get());
    ...
    public ConsistencyCheckResult checkDataConsistency() {
        ...
        for (SubClass subClass : subClasses) {
            List<Student> students = studentRepository.findByStuClassId(subClass);
            for (Student student : students) {
                if (student.getStuClassId() == null || !student.getStuClassId().getSubClassId().equals(subClass.getSubClassId())) {
            ...
            for (Major major : majors) {
                ...
                for (TotalClass totalClass : totalClasses) {
                    ...
                    for (SubClass subClass : subClasses) {
                        ...
                        // 获取学生数量
                        long studentCount = studentRepository.countByStuClassId(subClass);
    ...
    private CascadeOperationDTO.DeletePreviewDTO getSubClassDeletePreview(Integer subClassId, CascadeOperationDTO.DeletePreviewDTO preview) {
        ...
        // 统计影响的数据
        long studentCount = studentRepository.countByStuClassId(subClass.get());
        int totalAffected = 0;
